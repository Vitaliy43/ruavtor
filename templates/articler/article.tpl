...
{/if}
{if $activity == 2}
	<p><em>{time_convert_date_to_text($data_published)}</em></p>
{else:}
	<p><em>{time_convert_date_to_text($data_saved)}</em></p>
{/if}
...
{if isset($author_name)}
	<p style="text-align: left;">Автор: <strong><a href="{site_url('avtory')}/{$username}" style="color:black;text-decoration:underline;" target="_blank">{$author_name} {$author_family}</a></strong>
	{if empty($is_select_headings)}
<br />	Рубрика: <strong><a href="{site_url('')}{$headings_url}" style="color:black;text-decoration:underline;" target="_blank">{$headings}</a></strong>
{/if}
	 </p>

{else:}
	<p style="text-align: left;">Автор: <strong><a href="{site_url('avtory')}/{$username}" style="color:black;text-decoration:underline;" target="_blank">{$username}</a></strong>
	{if empty($is_select_headings)}
<br />	Рубрика: <strong>{$headings}</strong>
{/if}
{/if}
{if isset($data_published)}
<!--div id="data_published">Дата публикации: <font style="font-size:11px;"><b>{time_convert_date_to_text($data_published)}</b></font></div-->
{/if}
{if isset($rating)}
<p style="text-align: right;margin-top:-10px;" id="article_rating">Рейтинг: <strong>{$rating}</strong></p>
...
{/if}
{if isset($is_edited)}
<div style="margin-top:5px;">Аннотация (150 - 300 символов)</div>
<div>
...
<tr>
<td nowrap="">Внешняя ссылка:</td>
<td nowrap=""><a href="{$outer_link.link}" target="_blank">{$outer_link.link}</a></td>
<td align="left" nowrap=""><input type="checkbox" id="enable_link" onclick="set_link();">&nbsp;Закрепить ссылку</td>
</tr>
...
<td nowrap="">Начальный рейтинг (не более {$buffer} балл.)</td>
<td><input type="text" id="initial_rating" value="0" name="initial_rating"/>
</td>
<td></td>
</tr>
<tr class="pay_for_link">
<td nowrap="">Денежная добавка</td>
...
<div style="margin-bottom:10px;">
	...
	<span>
		Добавить бонус при выходе из песочницы ({$bonus_for_homefeed} балл.)
	</span>
</div>
...
<div style="margin-bottom:10px;"><a href="{site_url('moderator/edit')}/{$id}" target="_blank">Править статью</a></div>
{/if}
