...
//            return json_encode(array('result' => $result));
    ...
    function login() {
		...
		$this->template->assign('site_title', lang('lang_login_page'));
    ...
    function register() {
		...
		$is_ban = $this->articler_model->ban_after_register;

        $this->load->library('Form_validation');
		$this->template->assign('site_title', lang('lang_register'));
        ...
        if (!$this->dx_auth->is_logged_in() AND $this->dx_auth->allow_registration) {
            ...
            if ($val->run() AND $this->dx_auth->register($this->input->post('username'), $val->set_value('password'), $val->set_value('email'))) {
				...
				$res = $this->db->get_where('users',array('username' => $_POST['username']));
				if(!$is_ban)
					$this->articler_users->unban_user($res->row()->id);
				if(isset($_COOKIE['refer_id']) && isset($_COOKIE['refer_data'])){
					if($res){
						if($res->row()->id)
							$this->articler_users->create_refer($res->row()->id,$_COOKIE['refer_id'],$_COOKIE['refer_data']);
					}
				}
